...
object AccessLogStatHour2 {
  def main(args: Array[String]): Unit = {
   ...
   // val spark = SparkSession.builder().enableHiveSupport().getOrCreate()
    ...
    val accessAndIpSql =
    s"""
       |select a.*, udf_topDomain(a.domain) topdomain,
   ...
   // spark.sql(accessAndIpSql).show()
    ...
    val accessDomainTable = "accessDomainTable"
    ...
    spark.sql(accessDomainSql).repartition(10).write.mode(SaveMode.Overwrite).format("orc").save(domainPath)
